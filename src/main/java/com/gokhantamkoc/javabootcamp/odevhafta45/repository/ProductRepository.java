...
public class ProductRepository {
    ...
    // In order for the tests to pass, a constructor was added that takes
    // the DatabaseConnection parameter instead of a constructor without parameters.
    ...
    public void save(Product product) throws RuntimeException {
        ...
        try (PreparedStatement preparedStatement = databaseConnection.getConnection().prepareStatement(SQL)) {
            ...
            if (affectedRows <= 0) {
                throw new RuntimeException(String.format("Could not save product %s!", product.getDescription()));
            }
        } catch (Exception ex) {
            ex.printStackTrace();
            throw new RuntimeException(ex.getMessage());
